...
procinit(void)
{
  ...
  for(p = proc; p < &proc[NPROC]; p++) {
      ...
      if(pa == 0)
        panic("kalloc");
      word_t va = KSTACK((int) (p - proc));
      kvmmap(va, (word_t)pa, PGSIZE, PTE_R | PTE_W);
      p->kstack = va;
  }
  kvminithart();
}
...
proc_pagetable(struct proc *p)
{
  ...
  mappages(pagetable, TRAMPOLINE, PGSIZE,
           (word_t)trampoline, PTE_R | PTE_X);

  // map the trapframe just below TRAMPOLINE, for trampoline.S.
  mappages(pagetable, TRAPFRAME, PGSIZE,
           (word_t)(p->tf), PTE_R | PTE_W);
...
void
proc_freepagetable(pagetable_t pagetable, word_t sz)
{
...
uchar initcode[] = {
#ifdef __RV32__
  0x17,0x05,0x00,0x00,0x13,0x05,0xe5,0x01,0x97,0x05,0x00,0x00,0x93,0x85,0x05,0x02,
  0x9d,0x48,0x73,0x00,0x00,0x00,0x89,0x48,0x73,0x00,0x00,0x00,0xed,0x3f,0x2f,0x69,
  0x6e,0x69,0x74,0x00,0x00,0x01,0x00,0x13,0x1e,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
...
int
either_copyout(int user_dst, word_t dst, void *src, word_t len)
{
  struct proc *p = myproc();
  if(user_dst){
...
// Returns 0 on success, -1 on error.
int
either_copyin(void *dst, int user_src, word_t src, word_t len)
{
  struct proc *p = myproc();
  if(user_src){
